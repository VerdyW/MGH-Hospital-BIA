...
def build_fact_encounter(encounters_df: pd.DataFrame,
    ...
    fact["encounter_class_id"] = fact["encounter_class"].map(enc_class_map).astype(int)
    fact["clinical_code_id"]   = fact["reason_code"].astype(str).map(clinical_map).astype(int)
    ...
    fact["start_time_id"]      = _time_key(fact["start_time"]).astype(int)
    fact["stop_time_id"]       = _time_key(fact["stop_time"]).astype(int)
    fact['encounter_count']    = 1
    fact['encounter_count']    = fact['encounter_count'].astype(int)
...
def build_fact_procedures(procedures_df: pd.DataFrame,
    ...
    fact["procedure_type_id"] = fact["code"].astype(str).map(proc_map).astype(int)
    fact["clinical_code_id"]  = fact["reason_code"].astype(str).map(clinical_map).astype(int)
    fact["start_date_id"]     = pd.to_datetime(fact["start_time"]).dt.strftime("%Y%m%d").astype(int)
    fact["stop_date_id"]      = pd.to_datetime(fact["stop_time"]).dt.strftime("%Y%m%d").astype(int)
    fact["start_time_id"]     = _time_key(fact["start_time"]).astype(int)
    ...
    fact["procedure_count"]   = fact["procedure_count"].astype(int)
    ...
    fact['procedure_id'] = fact['procedure_id'].astype(int)
...
def build_fact_diagnosis(encounters_df: pd.DataFrame,
    ...
    fact["diagnosis_count"]     = fact["diagnosis_count"].astype(int)
...
def build_fact_billing(encounters_df: pd.DataFrame,
    ...
    fact = encounters_df.copy()
    fact["encounter_class_id"]      = fact["encounter_class"].map(enc_class_map).astype(int)
    fact["clinical_code_id"]        = fact["reason_code"].astype(str).map(enc_clinical_map).astype(int)
    fact["start_date_id"]           = pd.to_datetime(fact["start_time"]).dt.strftime("%Y%m%d").astype(int)
    fact["stop_date_id"]            = pd.to_datetime(fact["stop_time"]).dt.strftime("%Y%m%d").astype(int)
